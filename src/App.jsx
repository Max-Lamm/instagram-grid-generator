...
function App() {
  ...
  const [cropZoomPercent, setCropZoomPercent] = useState(100); // 0-200%+
  const [cropZoomInput, setCropZoomInput] = useState('100');
  const [minZoomPercent, setMinZoomPercent] = useState(0);
  const [maxZoomPercent, setMaxZoomPercent] = useState(200);
  ...
  // Calculate zoom scale that fills the frame (minimum zoom for background)
  const calculateFillZoom = (imgWidth, imgHeight, targetWidth, targetHeight) => {
    const scaleX = targetWidth / imgWidth;
    const scaleY = targetHeight / imgHeight;
    // For background: must fill entire frame (use max)
    // For poster: fit within frame (use min) - but we'll handle poster differently
  ...
  const handleFileSelect = (file, type) => {
    ...
    reader.onload = (event) => {
      const img = new Image();
      img.onload = () => {
        const { width: targetWidth, height: targetHeight } = type === 'background' 
          ? { width: TARGET_BG_WIDTH, height: TARGET_BG_HEIGHT }
          : { width: POSTER_WIDTH, height: POSTER_HEIGHT };
        
        // Calculate initial zoom to fill frame
        const fillScale = calculateFillZoom(img.width, img.height, targetWidth, targetHeight);
        const fillPercent = fillScale * 100;
        
        // For background: minimum zoom is fillPercent (can't show black)
        // For poster: minimum zoom is 0 (can have transparent areas)
        const minPercent = type === 'background' ? fillPercent : 0;
        
        // Maximum zoom: at least 200%, but extend if needed
        const maxPercent = Math.max(200, fillPercent * 2);
        
        setCropImageSrc(event.target.result);
        setCropImageObj(img);
        setCropType(type);
        setCropZoomPercent(fillPercent);
        setCropZoomInput(fillPercent.toFixed(0));
        setMinZoomPercent(minPercent);
        setMaxZoomPercent(maxPercent);
  ...
  const handleZoomInputBlur = () => {
    let value = parseFloat(cropZoomInput);
    if (isNaN(value)) {
      value = cropZoomPercent;
    } else {
      value = Math.max(minZoomPercent, Math.min(maxZoomPercent, value));
    }
    setCropZoomPercent(value);
    setCropZoomInput(value.toFixed(0));
  };
  ...
  // Constrain position to prevent black borders (for background)
  const constrainPosition = (pos, zoomScale, imgWidth, imgHeight, targetWidth, targetHeight) => {
    if (cropType !== 'background') return pos; // No constraints for poster
    
    const scaledWidth = imgWidth * zoomScale;
    const scaledHeight = imgHeight * zoomScale;
    
    // Calculate max offset in each direction
  ...
  useEffect(() => {
    if (!cropModalOpen || !cropImageSrc || !cropImageObj) return;
    ...
    const zoomScale = cropZoomPercent / 100;
    ...
    if (constrainedPos.x !== cropPosition.x || constrainedPos.y !== cropPosition.y) {
      setCropPosition(constrainedPos);
    }
    ...
    if (cropType === 'background') {
      ...
      // Transparent background for poster
      ctx.clearRect(0, 0, canvas.width, canvas.height);
      // Draw checkerboard pattern to show transparency
      const checkSize = 10;
      for (let y = 0; y < canvas.height; y += checkSize) {
        for (let x = 0; x < canvas.width; x += checkSize) {
          ctx.fillStyle = ((x / checkSize + y / checkSize) % 2 === 0) ? '#fff' : '#e0e0e0';
    ...
    const x = (canvas.width / 2) + (constrainedPos.x * displayScale);
    const y = (canvas.height / 2) + (constrainedPos.y * displayScale);
  ...
  }, [cropModalOpen, cropImageSrc, cropImageObj, cropZoomPercent, cropPosition, cropType]);
  ...
  const handleCropConfirm = () => {
    ...
    // Constrain position one final time
    const constrainedPos = constrainPosition(
      cropPosition,
      zoomScale,
      img.width,
    ...
    const x = (canvas.width / 2) + constrainedPos.x;
    const y = (canvas.height / 2) + constrainedPos.y;
  ...
  const handleCanvasMouseMove = (e) => {
    ...
    const newPos = {
      x: cropPosition.x + deltaX / 0.3,
      y: cropPosition.y + deltaY / 0.3
    };
    
    // Constrain if background
    const { width: targetWidth, height: targetHeight } = getTargetDimensions();
    const zoomScale = cropZoomPercent / 100;
    const constrainedPos = constrainPosition(
      newPos,
          ...
          <div className="bg-white rounded-2xl p-6 max-w-4xl w-full">
            ...
            <div className="mb-6">
              <div className="flex items-center gap-4 mb-3">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setCropZoomPercent(val);
                    setCropZoomInput(val.toFixed(0));
                  }}
                  className="flex-1"
                />
              ...
              <div className="flex items-center justify-between text-xs text-gray-500">
                <span>{minZoomPercent.toFixed(0)}% (min)</span>
                <span>100% (original)</span>
                <span>{maxZoomPercent.toFixed(0)}% (max)</span>
              </div>
